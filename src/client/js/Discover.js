...
const renderDiscover = () => {
  ...
  // create form
  const inputs = [
    { title: "location", placeholder: "city, state" },
    { title: "start date", placeholder: "MM/DD/YYYY" },
    { title: "end date", placeholder: "MM/DD/YYYY" },
  ];
  ...
  for (const title of inputs) {
    ...
    input.setAttribute("placeholder", `${title.title}: ${title.placeholder}`);
    //creates classname without spaces
    const regexTitle = title.title.replace(/\s+/g, "");
      ...
      switch (title.title) {
        case "location":
          ...
          regLocation.test(input.value)
            ? (state[regexTitle] = input.value) && valid(title.title)
            : invalid(title.title);
          break;
        default:
